...
fn test_anti_aliasing_smoothes_edges() {
    ...
    let img_1spp = render_image_parallel(&hairy_tracer_core::render::WhittedIntegrator, &scene, cam_origin, look_at, up, 1.0, 1.0, 1.0, 4, 1, 1, 1, 0.0, 5.0,
    );
    ...
    // Render at 100 SPP
    let img_100spp = render_image_parallel(&hairy_tracer_core::render::WhittedIntegrator, &scene, cam_origin, look_at, up, 1.0, 1.0, 1.0, 4, 1, 1, 100, 0.0, 5.0,
    );
...
fn test_soft_shadows_penumbra() {
    ...
    let img_hard = render_image_parallel(&hairy_tracer_core::render::WhittedIntegrator, &scene, cam_origin, look_at, up, 1.0, 5.0, 5.0, 20, 20, 1, 10, 0.0, 5.0,
    );

    // Make light soft
    scene.lights[0].radius = 1.0;
    let img_soft = render_image_parallel(&hairy_tracer_core::render::WhittedIntegrator, &scene, cam_origin, look_at, up, 1.0, 5.0, 5.0, 20, 20, 1, 100, 0.0, 5.0,
    );
...
fn test_dof_blurs_off_focal_plane() {
    ...
    let img_focus_near = render_image_parallel(&hairy_tracer_core::render::WhittedIntegrator, &scene, cam_origin, look_at, up, 1.0, 2.0, 2.0, 20, 20, 1, 50, 1.0, 5.0,
    );

    // 2. Focus at z=0 (focal_distance = 10). Far sphere is sharp, near sphere is blurred.
    let img_focus_far = render_image_parallel(&hairy_tracer_core::render::WhittedIntegrator, &scene, cam_origin, look_at, up, 1.0, 2.0, 2.0, 20, 20, 1, 50, 1.0, 10.0,
    );
